...
<v-list-item 
  v-for="chef in chefsProjet" 
  :key="chef.idUtilisateur"  
  @click="initiateChat(chef)" 
  class="cursor-pointer">
  <v-row align="center">
    <v-col cols="2" class="ma-1">
      <v-avatar size="45">
        <v-img :src="getImageUrl(chef.profile)"></v-img>
      </v-avatar>
    </v-col>
    ...
    <v-col cols="auto" v-if="unreadMessages[chef.idUtilisateur] > 0">
      <v-badge color="red" :content="unreadMessages[chef.idUtilisateur]" />
    </v-col>
  </v-row>
</v-list-item>
...
export default {
  ...
  methods: {
    ...
    async fetchUsers() {
  try {
    const response = await axios.get('http://localhost:5000/admin/projectManager', {
      params: { idUtilisateur: this.userId }
    });

    // Accédez à la bonne clé 'data' dans la réponse
    this.chefsProjet = response.data.data; // Utilisez 'data' au lieu de 'chefsProjet'

    // Initialiser les messages non lus pour chaque chef de projet
    this.chefsProjet.forEach(user => {
